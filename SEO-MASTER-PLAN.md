...
- [ ] **1.6 Measurement.** BLOCKED ON USER 2026-08-01: no GA4 property exists yet for amautdc.com. User needs to create one (see instructions below) and provide the Measurement ID so tracking + conversion events (quote-form submit, `tel:` clicks) can be wired in. GSC verification file (`google11c7e5ed08484531.html`) is already in place — confirm in Search Console that the sitemap is submitted and the extensionless URL set is what's indexed (not `.html` duplicates).

  **To create GA4 (5 minutes):** go to [analytics.google.com](https://analytics.google.com) → Admin → Create Property → name it "AMA United Company" → fill in business details → choose "Web" as the platform → enter `https://amautdc.com` → this generates a **Measurement ID** in the form `G-XXXXXXXXXX` (Admin → Data Streams → click the web stream). Bring that ID to the next session and tracking will be wired in immediately.
